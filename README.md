...
Вопросы о погоде (`какая погода в Москве?`, `сколько градусов в Питере?`)
обрабатываются **напрямую через метео-API (Open-Meteo)** — быстро и без
зависимости от веб-поиска. Поддерживаются основные города РФ и СНГ.

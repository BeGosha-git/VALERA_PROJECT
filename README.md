...
|--------|--------|---------------------------|
| `Qwen/Qwen2.5-Omni-3B` **(сейчас, для скорости)** | ~7.6 GB (fp16) | ✅ Да, **9.6 ток/с** |
| `Qwen/Qwen2.5-Omni-7B` | ~20.8 GB (bf16/fp16) | ✅ Да, 4.3 ток/с (в 2.3 раза медленнее) |
...
### Скорость генерации (замеры на Jetson AGX Orin, режим MAXN)

| Что | 7B | 3B |
|-----|----|----|
| Скорость генерации | 3.7–4.3 ток/с | **9.6 ток/с** |
| Ответ 15 токенов | ~4 с | **~2.5 с** |
| Ответ 40 токенов | ~10 с | ~4 с |

Поиск (тоже замерено): **база документов 0.08 с**, **интернет ~2 с** — итого ~2 с.

> 💡 **Про 2 секунды.** Генерация линейно зависит от длины ответа, поэтому
> «до 2 с» = примерно 15 токенов (~10 слов). Если нужно строже — сократите лимит
> в персоне (`core/personas.py`, `DEFAULT_*_PROMPT`) до 12-15 слов.
> Модель 3B + режим MAXN — самый быстрый вариант без квантизации.
...
Смена модели — в `.env`:
```ini
VALERA_MODEL_NAME_OR_PATH=Qwen/Qwen2.5-Omni-3B   # быстрее (рекомендуется)
# VALERA_MODEL_NAME_OR_PATH=Qwen/Qwen2.5-Omni-7B  # качественнее, но в 2.3 раза медленнее
```

> ⚠️ Скорость работы (ток/с) зависит от режима питания Jetson:
> ```bash
> sudo nvpmodel -m 0 && sudo jetson_clocks   # MAXN + максимальные частоты
> ```
> Также `VALERA_MODEL_DTYPE=float16` примерно на 15% быстрее, чем bf16.
...
после секунды тишины, сразу отправляет аудио и **сразу начинает говорить ответ**
(по умолчанию — стримингом, по 4 слова, пока модель думает дальше).
Нажимать ничего не нужно — выход по Ctrl+C.

```bash
python client.py --mode voice                 # авто-режим + стриминг (по умолчанию)
python client.py --mode voice --chunk-words 6 # озвучивать порциями по 6 слов
python client.py --mode voice --no-stream     # ждать весь ответ целиком
python client.py --mode voice --mat           # режим с матом
...
### ⚡ Стриминг: почему ответ звучит сразу

Ответ не ждётся целиком — он генерируется и сразу озвучивается порциями:

```
0.6 с   первый токен текста (для текстового вопроса)
~4 с    первый токен при ВОПРОСЕ ГОЛОСОМ (кодирование аудио — самая дорогая часть)
+2 с    Silero синтезирует первый кусок (4 слова)
≈ 6 с   первое слово из динамика — и дальше звук идёт без паузы,
        пока модель дописывает остальное
```

Что внутри:

* генерация идёт в отдельном потоке и отдаётся через SSE-события
  (иначе event loop блокируется и клиент получает всё в конце);
* текст режется на куски по ``--chunk-words`` слов (по умолчанию 4) — первый
  кусок маленький для быстрого старта, следующие крупнее для плавной интонации;
* Silero синтезирует ~2 с на каждую секунду речи, поэтому всё считается
  параллельно с генерацией:

```mermaid
sequenceDiagram
    participant M as Модель (GPU)
    participant S as Silero (CPU)
    participant P as 🔊 Динамики
    M->>S: "В нашей солнечной системе"
    S->>P: звучит (1.8 с)
    M->>S: "семь планет." (дописывает дальше)
    S->>P: звучит сразу следом
```

Служебные фразы («спрашивай, если что», «feel free to ask») в озвучку не идут —
они отсекаются по предложениям.
...
| `POST` | `/api/v1/chat/voice/stream` | **Стриминг**: текст + озвучка порциями (SSE) |
...
Измерено на реальном запросе (вопрос голосом 1.9 с → ответ голосом):

| | всего | подготовка | генерация | синтез речи |
|---|---|---|---|---|
| до оптимизаций | 15.2 с | 2.2 с | 7.8 с | 4.7 с |
| **сейчас** | **8.9 с** | 2.1 с | 3.4 с | 3.4 с |

Ускорение в 1.7 раза дала обрезка служебных «хвостов» — модель по привычке
дописывала «Если у вас есть ещё вопросы, задавайте», а это лишние токены
генерации и секунды озвучки.

Встроенный Talker Qwen на том же запросе тратит 93 с вместо 15 — поэтому по
умолчанию используется Silero.
...
## 🚫 Запрет английского (только русский)

По умолчанию включён тег **только русский**: ассистент отвечает по-русски даже на
английский вопрос (смысл переводится, ответ — по-русски).

```ini
VALERA_RUSSIAN_ONLY=true    # по умолчанию — только русский
# VALERA_RUSSIAN_ONLY=false # двуязычно: англ. вопрос → англ. ответ
```

Работает в три слоя (одного промпта модели не хватает — проверено):

```mermaid
flowchart TD
    A["1. Персона: «Отвечай ТОЛЬКО по-русски,\nни одного английского слова»"] --> B["2. Напоминание прямо перед\nрепликой пользователя"]
    B --> C{"ответ на английском?"}
    C -->|да| D["3. Генерация заново\nс усиленным требованием"]
    C -->|нет| E["готово"]
    D --> E
```

- слой 2 действует и в стриминге (там повтор уже невозможен — текст озвучен);
- слой 3 — страховка для обычных запросов: если ответ всё-таки вышел английским,
  он генерируется заново (в лог попадает «повторяю с усиленным требованием»).

## ⚙️ Конфигурация

### 🗣️ Нормализация речи (фильтр из ветки PC)

ASR часто слышит «МИРЭА» как «мир», «мире», «мирэ». Без нормализации модель
отвечает про мир вообще, а не про университет. Фильтр в `core/text_filters.py`:

```ini
VALERA_MIREA_AGGRESSIVE=false   # по умолчанию: только «мирэ», «мирэа»
                                # true: + обычные падежи «мир», «мире»,
                                #       но тогда «во всём мире» → «во всём МИРЭА»
```

```python
from core.text_filters import normalize_mirea, is_mirea_related
normalize_mirea("расскажи про мирэа")   # → 'расскажи про МИРЭА'
is_mirea_related("расскажи про институт")  # → True
```

`is_mirea_related()` отвечает на вопрос «похоже ли это на запрос про вуз» по
списку слов (институт, лаборатория, вуз, стромынка, вернадка, РТУ…).

...
class ChatService:
    ...
    def __init__(self):
        ...
        # Connect to MongoDB (или локальное JSON-хранилище, если Mongo нет)
        try:
            from app.services.local_store import get_client
            self.client = get_client(self.mongo_uri)
            self.db = self.client[self.db_name]
            self.chats = self.db[self.chats_collection_name]
            self.messages = self.db[self.messages_collection_name]
            logger.info(
                "Chat store: %s",
                "MongoDB" if self.client.__class__.__name__ == "MongoClient" else "local JSON",
            )

...
# Docling importы для продвинутой обработки документов.
# Зависимость тяжёлая и опциональная: если её нет, приложение всё равно
# стартует, а для PDF используется запасной путь через PyPDF2.
try:
    ...
    class InputFormat:
        """Заглушка атрибутов InputFormat, когда docling не установлен."""

        PDF = DOCX = PPTX = XLSX = HTML = MD = ASCIIDOC = CSV = XML_JATS = IMAGE = None
...
class DocumentService:
    ...
    def __init__(self):
        ...
        # (только если docling установлен — иначе работаем через PyPDF2)
        self.converter = None
        if DOCLING_AVAILABLE:
            ...
            logger.warning(
                "docling не установлен — конвертация Office-форматов недоступна, "
                "для PDF используется PyPDF2"
            )

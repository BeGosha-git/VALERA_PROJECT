...
# NOTE: openai / anthropic импортируются лениво внутри соответствующих методов —
# при работе с локальной моделью (провайдер «valera») эти пакеты не нужны.
...
class ModelService:
        ...
        if self.config_path.exists():
            try:
                with open(self.config_path, 'r') as f:
                    ...
                    if os.getenv('VALERA_API_BASE') and 'valera' not in config['providers']:
                        config['providers']['valera'] = {
                            "models": [],
                            "config": {"api_base_env": "VALERA_API_BASE"}
                        }
    ...
    def _create_initial_config(self):
        ...
        # Локальная модель Qwen2.5-Omni (наш сервер VALERA_PROJECT) — замена Ollama.
        # По умолчанию включена, т.к. модель живёт на этой же машине.
        if os.getenv('VALERA_API_BASE'):
            config['providers']['valera'] = {
                "models": [],
                "config": {"api_base_env": "VALERA_API_BASE"}
            }
            # Приоритет выше Ollama: локальная модель не требует внешних сервисов
            if not config['active']['llm_provider'] or config['active']['llm_provider'] == 'ollama':
                config['active']['llm_provider'] = 'valera'
    ...
    def _update_available_models(self):
        ...
        # Локальная модель Qwen2.5-Omni (замена Ollama)
        if os.getenv('VALERA_API_BASE') and 'valera' in self.config["providers"]:
            try:
                self._update_valera_models()
            except Exception as e:
                logger.warning(f"Error updating VALERA models: {e}")
    ...
    def _cleanup_unavailable_providers(self):
        ...
        # Check if VALERA API base is no longer available
        if 'valera' in self.config["providers"] and not os.getenv('VALERA_API_BASE'):
            providers_to_remove.append('valera')
        ...
        if providers_to_remove and self.config["active"]["llm_provider"] in providers_to_remove:
            ...
            if 'valera' in self.config["providers"]:
                available_providers.append('valera')
    ...
    def _update_valera_models(self):
        """Обновляет список моделей локального сервера VALERA_PROJECT.

        Наш API не отдаёт список моделей (на GPU живёт ровно одна), поэтому
        берём имя из VALERA_MODEL_NAME_OR_PATH и проверяем, что сервер жив.
        """
        api_base = os.getenv('VALERA_API_BASE', 'http://localhost:8765/api/v1').rstrip('/')
        model_name = os.getenv('VALERA_MODEL_NAME_OR_PATH', 'Qwen/Qwen2.5-Omni-7B')

        # Сохраняем признак default у уже выбранной модели
        is_default = False
        for existing_model in self.config['providers']['valera']['models']:
            if existing_model['id'] == model_name:
                is_default = existing_model.get('default', False)
                break
        ...
        models = [{
            ...
            "name": model_name,
            "type": "llm",
            "default": is_default or not self.config['providers']['valera']['models'],
        }]
        ...
        try:
            ...
            if resp.status_code != 200 or not resp.json().get('model_loaded'):
                logger.warning(
                    "VALERA API is up but the model is not loaded yet: %s", api_base
                )
        except Exception as e:
            logger.warning(f"Could not reach VALERA API at {api_base}: {e}")

        self.config['providers']['valera']['models'] = models
    ...
    def get_available_providers(self):
        ...
        # Локальная модель Qwen2.5-Omni (замена Ollama)
        valera_api = os.getenv('VALERA_API_BASE')
        if valera_api:
            try:
                resp = requests.get(f"{valera_api.rstrip('/')}/health", timeout=2)
                loaded = resp.status_code == 200 and resp.json().get('model_loaded')
                providers.append({
                    "id": "valera",
                    "name": "Qwen2.5-Omni (локально)",
                    "available": bool(loaded),
                    **({} if loaded else {
                        "reason": f"Сервер {valera_api} не готов (модель ещё грузится?)"
                    }),
                })
            except Exception:
                providers.append({
                    ...
                    "name": "Qwen2.5-Omni (локально)",
                    "available": False,
                    "reason": f"Нет связи с сервером {valera_api} — запустите python main.py",
                })

...
# Обёртка вокруг SentenceTransformer: device задаём ВСЕГДА явно.
#
#  1) В Jetson-сборке torch отсутствует torch.distributed, а
#     sentence-transformers без явного device вызывает get_device_name(),
#     который обращается к torch.distributed.is_initialized() →
#     AttributeError: module 'torch.distributed' has no attribute 'is_initialized'.
#  2) GPU должен оставаться свободным для Qwen2.5-Omni: эмбеддинги маленькие
#     (e5-small), их считаем на CPU.
SentenceTransformer = partial(
    _SentenceTransformer, device=os.getenv('EMBEDDING_DEVICE', 'cpu')
)
...
class QdrantService:
    ...
    def __init__(self):
        ...
        # Jetson-вариант: сервер Qdrant может отсутствовать (нет Docker).
        # qdrant-client умеет работать локально с папкой на диске —
        # используем это как прозрачный фолбэк.
        self.local_path = os.getenv('QDRANT_LOCAL_PATH')
        self.client = None
        if not self.local_path:
            try:
                client = QdrantClient(host=self.host, port=self.port, timeout=3)
                client.get_collections()
                self.client = client
            except Exception as e:
                self.local_path = str(Path('data/qdrant_storage'))
                print(
                    f"Qdrant server at {self.host}:{self.port} is unavailable ({e}). "
                    f"Falling back to local storage: {self.local_path}"
                )
